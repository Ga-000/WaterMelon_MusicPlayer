...
//				String path = "file:/C:/Users/82104/git/watermelon/musicList/";
				String path = "file:/C:/javas/java-workspace/waterMelon/musicList/";
				System.out.println("title : " + PlaytitleDTO.getTitle());
...
//		System.out.println("musicPresent Dao " + Dao);
		...
		for (File file : songs) {
			...
			if (PlaytitleDTO.getTitle().equals(SongTitle)) {
				...
				if(SongTitle.equals("그라데이션")) { SongSinger="10CM"; SongTitle="그라데이션"; }
				if(SongTitle.equals("낙하")) { SongSinger="AKMU (악뮤) (with 아이유)"; SongTitle="낙하"; }
				if(SongTitle.equals("LovesickGirls")) { SongSinger="BLACKPINK"; SongTitle="Lovesick Girls"; }
				if(SongTitle.equals("MyUniverse")) { SongSinger="Coldplay"; SongTitle="My Universe"; }
				if(SongTitle.equals("AfterLIKE")) { SongSinger="IVE (아이브)"; SongTitle="After LIKE"; }
				if(SongTitle.equals("WhenWillMyLifeBegin")) { SongSinger="Mandy Moore"; SongTitle="When Will My Life Begin"; }
				if(SongTitle.equals("Ditto")) { SongSinger="NewJeans"; SongTitle="Ditto"; }
				if(SongTitle.equals("나의X에게")) { SongSinger="경서"; SongTitle="나의 X에게"; }
				if(SongTitle.equals("선물")) { SongSinger="멜로망스"; SongTitle="선물"; }
				if(SongTitle.equals("야생화")) { SongSinger="박효신"; SongTitle="야생화"; }
				if(SongTitle.equals("밝게빛나는별이되어비춰줄게")) { SongSinger="송이한"; SongTitle="밝게 빛나는 별이 되어 비춰줄게"; }
				if(SongTitle.equals("strawberrymoon")) { SongSinger="아이유"; SongTitle="strawberry moon"; }
				if(SongTitle.equals("비와당신(슬기로운의사생활시즌2OSTPart1)")) { SongSinger="이무진"; SongTitle="비와 당신 (슬기로운 의사생활 시즌2 OST Part 1)"; }
				if(SongTitle.equals("신호등")) { SongSinger="이무진"; SongTitle="신호등"; }
				if(SongTitle.equals("Monologue")) { SongSinger="테이"; SongTitle="Monologue"; }
				if(SongTitle.equals("커피한잔할래요")) { SongSinger="폴킴"; SongTitle="커피 한 잔 할래요"; }
				
				songLabel.setText(SongTitle);
				singer.setText(SongSinger);
	...
	public void sliderTime() {
		...
		Thread th = new Thread(() -> {
			...
			while (currentT < totalT) {
				...
				currentT = (double) mediaPlayer.getCurrentTime().toSeconds();
				Platform.runLater(() -> {
...
//				opener.alertopen("플레이 리스트에 추가하였습니다.");
				...
				cdao.insertMYlist(songTitle, songSinger);
			} 
//			else {
